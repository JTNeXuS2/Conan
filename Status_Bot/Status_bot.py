...
def update_settings():
    global token, channel_id, message_id , update_time, bot_name, bot_ava, address, command_prefex, servers_list
    ...
    if config:
        try:
            ...
            # Формируем список серверов из секции [servers]
            if 'servers' in config:
                servers_list = []
                for idx, (name, addr) in enumerate(config['servers'].items(), 1):
                    if ':' in addr:
                        ip, port_str = addr.split(':', 1)
                        try:
                            port = int(port_str)
                        except ValueError:
                            print(f"Warning: неверный порт у сервера {name}: {port_str}")
                            port = None
                    else:
                        ip = addr
                        port = None
                    servers_list.append((idx, name, ip, port))
...
async def update_server_data():
    global server_infos, player_count
    update_settings()

    player_count = 0
    temp_infos = []

    if update_server_data.seconds != int(update_time):
        update_server_data.change_interval(seconds=int(update_time))

    for idx, name, ip, port in servers_list:
        address = (ip,) if port is None else (ip, port)
        try:
            info, players, rules = await get_info(address)
            cur_players = len(players)
            #print(f"cur_players>>{cur_players}")
            max_players = info.max_players
            player_count += cur_players
            temp_infos.append((name, cur_players, max_players))
        except Exception as e:
            #normal_time = datetime.datetime.utcfromtimestamp(player.duration).strftime('%H:%M:%S')
            normal_time = datetime.datetime.utcnow().strftime('%H:%M:%S')
            print(f"[{normal_time}] Ошибка при опросе сервера {name}: {e}")
            temp_infos.append((name, '', "❌ Offline"))
    #print(f"players>>{player_count}")
    server_infos = temp_infos
...
async def cycle_status():
    ...
    if not server_infos:
        # Если данных нет, показываем простой статус
        await bot.change_presence(status=disnake.Status.online, activity=disnake.Game(name="Серверы недоступны"))
        return

    for name, cur_players, max_players in server_infos:
        status_text = f"{name}: {cur_players}/{max_players} | All: {player_count}"
...
async def on_ready():
    ...
    # Запускаем оба цикла
    update_server_data.start()
    cycle_status.start()

    # Обновляем имя и аватар бота один раз при старте
...
async def shutdown_bot():
    update_server_data.cancel()
    cycle_status.cancel()
    await bot.close()

crosschat_id = "1414042942724767754"
@bot.event
async def on_message(message):
    if message.author == bot.user:		#отсеим свои сообщения
        return;
